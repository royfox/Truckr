...
<div class="users-column">
    <div class="user-thumb">
        <div class="gravatar">
            <?php echo $this->Gravatar->image($post['User']['email'], array('size' => 80), array('alt' => 'Gravatar', 'default'=>'identicon')); ?>
        </div>
    ...
    <div class="subscribers">
        ...
        <?php if(count($post['Subscriber'])):?>
            <?php foreach($post['Subscriber'] as $subscriber):?>
                <?php echo $this->Gravatar->image($subscriber['User']['email'], array('size' => 33), array('alt' => 'Gravatar', 'default'=>'identicon', 'title' => $subscriber['User']['display_name'].' is subscribed to ths post')); ?>&nbsp;
            <?php endforeach;?>
        <?php else:?>
        ...
        <div class="links">
            <?php if(in_array($current_user['id'], $subscriber_ids)):?>
                ...
                <?php echo $this->Html->link('Unsubscribe', array('controller' => 'subscribers','action' => 'delete', $post['Post']['id']), array('class'=>'btn btn-small'));?>
            </span>
            <?php else:?>
                <span class="manage_subscription subscribe">
                <?php echo $this->Html->link('Subscribe', array('controller' => 'subscribers','action' => 'add', $post['Post']['id']), array('class'=>'btn btn-small'));?>
            </span>
            <?php endif;?>
        </div>
    </div>
    ...
    <div class="comments">
        <?php foreach($post['Comment'] as $comment):?>
            <div class="comment">
                ...
                <div class="user-thumb user-thumb-comment">
                    <div class="gravatar">
                        <?php echo $this->Gravatar->image($comment['User']['email'], array('size' => 60), array('alt' => 'Gravatar', 'default'=>'identicon')); ?>
                    </div>
